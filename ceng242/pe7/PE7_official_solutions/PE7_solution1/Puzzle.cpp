...
/* It returns directly the piece object located in the sub-puzzle (cell)
   ...
   Exception.h file.
*/
const Piece& Puzzle::getPiece(int coords[2]) const {
	if (size == 1)
		return *piece;
...
Puzzle Puzzle::crop(int from[2], int to[2]) const {
	...
	if (this->first_row == from[0] && this->first_column == from[1] && this->first_row + size - 1 == to[0] && this->first_column + size - 1 == to[1])
		return *this;
	...
	Puzzle p(to[0] - from[0] + 1, from[0], from[1]);
	return p;
