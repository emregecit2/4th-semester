...
*/
Edge::Edge(int id, Password password): id(id), password(password), matchingEdge(NULL) {}
...
void Edge::breakMatch() {
    if (matchingEdge) {
        matchingEdge = matchingEdge->matchingEdge = NULL;
    }
}
...
Edge* StraightEdge::clone() const {
    StraightEdge* clone = new StraightEdge(id);
    if (matchingEdge) {
        StraightEdge* cloneMatch = new StraightEdge(matchingEdge->getId());
        clone->matchingEdge = cloneMatch;
        cloneMatch->matchingEdge = clone;
    }
    return clone;
}
...
bool StraightEdge::matchWithHelper(Password password) {
    return password == SEND_ME_STRAIGHT;
}
...
Edge* InwardsEdge::clone() const {
    InwardsEdge* clone = new InwardsEdge(id);
    if (matchingEdge) {
        InwardsEdge* cloneMatch = new InwardsEdge(matchingEdge->getId());
        clone->matchingEdge = cloneMatch;
        cloneMatch->matchingEdge = clone;
    }
    return clone;
...
bool InwardsEdge::matchWithHelper(Password password) {
    return password == SEND_ME_INWARDS;
}
...
Edge* OutwardsEdge::clone() const {
    OutwardsEdge* clone = new OutwardsEdge(id);
    if (matchingEdge) {
        OutwardsEdge* cloneMatch = new OutwardsEdge(matchingEdge->getId());
        clone->matchingEdge = cloneMatch;
        cloneMatch->matchingEdge = clone;
    }
    return clone;
...
bool OutwardsEdge::matchWithHelper(Password password) {
	return password == SEND_ME_OUTWARDS;
}
...
*/
CompositeEdge::CompositeEdge(int id): Edge(id, SEND_ME_COMPLETING_COMPOSITE), matchIndex(0) {}
