...
/* Constructor : initializes <id> of the object to
   ...
   later on.
*/
Student::Student(int _id, string _fullname, string _department): id(_id), fullname(_fullname), department(_department), upgrade(false) {}
...
void Student::gradeCourse(const OpenCourse& openCourse) {
	gpa = learnGrade(openCourse.getName(), id);
}
